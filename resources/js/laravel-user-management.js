...
$(function () {
        ...
        var dt_user = dt_user_table.DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    targets: 3,
                    render: function (data, type, full, meta) {
                        var $father_name = full['father_name'];

                        return '<span class="user-father_name">' + $father_name + '</span>';
                    }
                },
                {
                    targets: 4,
    ...
    $(document).on('click', '.edit-record', function () {
        ...
        $.get(`${baseUrl}user-list\/${user_id}\/edit`, function (data) {
            ...
            $('#add-user-father-name').val(data.father_name);
            $('#status').val(data.status).trigger('change');

            fv.removeField('password');
            fv.removeField('password_confirmation');

            // Optionally, set these fields to empty in case of edit
    ...
    $('.add-new').on('click', function () {
        ...
        fv.addField('password', {
            validators: {
                ...
                regexp: {
                    regexp: /^(?=.*[a-z])(?=.*[\d\s\W]).+$/,
                    message: 'Your password must contain at least one lowercase character, and at least one number, symbol, or whitespace character'
                }
            }
        });

        fv.addField('password_confirmation', {
            validators: {
                notEmpty: {
